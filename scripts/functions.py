...
def antialias(phin,imagepix,beam_ratio,Nact):
	...
	phfilt=np.abs(pupil_to_image(np.fft.fftshift(image_to_pupil(phinput))*(buttf(xy,1,Nact/2.*beam_ratio*0.99,100)))) #assuming 32 actuators
	phout=phfilt-np.mean(phfilt)
	return phout


def make_noise_pl(wavefronterror,imagepix,pupilpix,wavelength,pl,Nact):
	'''
	...
	norm_factor=(wavefronterror/wavelength*2.*np.pi)/np.std(antialias(noise_wavefront,imagepix,beam_ratio,Nact)[np.where(xy<pupilpix/2.)]) #normalization factor for phase error over the pupil of modes within the DM control region
...
def make_noise_pl_ho(wavefronterror,imagepix,pupilpix,wavelength,pl,Nact,lo_cutoff):
	'''
	make noise with a user input power law, specifically adding only high order aberrations (i.e., assuming a LOWFS separately takes care of the low order modes). lo_cutoff is the spatial frequency (in c/p) below which there are no aberrations added:

	(1) take white noise in image plane
	(2) IFFT to pupil plane
	(3) weight complex pupil plane (spatial frequency) by a sloped power law and mid to high spatial frequencies and zero at low to mid spatial frequencies
	(4) FFT back to image plane
	...
	#lo_cutoff=3
	amplitude[xy<lo_cutoff*beam_ratio]=0. #remove low to mid spatial frequencies
