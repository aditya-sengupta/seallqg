...
#DM aperture:
#rho,phi = ao_utils.polar_grid(xdim,xdim*29/32) #assuming 29 of the 32 actuators are illuminated
rho,phi = ao_utils.polar_grid(xdim,xdim*28/32) #assuming BLANK of the 32 actuators are illuminated
...
time.sleep(0.05)
imcen = stack(1000)
imxcenini,imycenini = 157.69,166.94

beam_ratio = 0.635*750/13.5/6.5 #theoretical number of pixels/resel: lambda (in microns)*focal length to camera (in mm)/non-coronagraphic beam diameter at the Lyot stop (in mm)/Andor pixel size (in microns); note initially I was using the coronagraphic Lyot stop diameter instead of the non-coronagraphic one, but the latter turns out to be much more accurate so this should be used moving forward.s

#fit the four sine spots with Gaussians to robustly set the image center
ycropmin = np.round(np.array([imycenini-(freq+2)*beam_ratio,imycenini-2*beam_ratio,imycenini+(freq-2)*beam_ratio,imycenini-2*beam_ratio])).astype(int)
ycropmax = np.round(np.array([imycenini-(freq-2)*beam_ratio,imycenini+2*beam_ratio,imycenini+(freq+2)*beam_ratio,imycenini+2*beam_ratio])).astype(int)
xcropmin = np.round(np.array([imxcenini-2*beam_ratio,imxcenini-(freq+2)*beam_ratio,imxcenini-2*beam_ratio,imxcenini+(freq-2)*beam_ratio])).astype(int)
xcropmax = np.round(np.array([imxcenini+2*beam_ratio,imxcenini-(freq-2)*beam_ratio,imxcenini+2*beam_ratio,imxcenini+(freq+2)*beam_ratio])).astype(int)
crop1 = imcen[ycropmin[0]:ycropmax[0],xcropmin[0]:xcropmax[0]]
crop2 = imcen[ycropmin[1]:ycropmax[1],xcropmin[1]:xcropmax[1]]
crop3 = imcen[ycropmin[2]:ycropmax[2],xcropmin[2]:xcropmax[2]]
crop4 = imcen[ycropmin[3]:ycropmax[3],xcropmin[3]:xcropmax[3]]
...
#ind_dh = np.where(np.logical_and(np.logical_and(Im_rho<(maxld+2)*beam_ratio,Im_rho>(minld-2)*beam_ratio),Im_grid[1]-imxcen<0))
...
ind_dh = np.where(np.logical_and(np.logical_and(Im_grid[1]-imxcen<-(minld-2)*beam_ratio,Im_grid[1]-imxcen>-(maxld+2)*beam_ratio),np.logical_and(Im_grid[0]-imycen<(ylimld+2)*beam_ratio,Im_grid[0]-imycen>-(ylimld+2)*beam_ratio))) #note the +2 padding, which accounts for the unknown beam ratio in not cutting off the fmask later in the IM code at the edges of the DH
dh_mask[ind_dh] = 1.
