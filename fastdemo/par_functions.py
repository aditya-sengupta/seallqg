...
ind_mask_dh=ind_mask_dh=np.where(np.logical_and(np.logical_and(grid[0]>imagepix/2.-N_act/2.*beam_ratio,grid[0]<imagepix/2.+N_act/2.*beam_ratio),np.logical_and(np.logical_and(grid[1]>imagepix/2.-N_act/2.*beam_ratio,grid[1]<imagepix/2.),xy_dh>(e+1)*beam_ratio))) #half DH ignoring Fourier modes close to the IWA
DH_mask=np.zeros((imagepix,imagepix))
DH_mask[ind_mask_dh]=1.
contrast = lambda im: np.nanstd(im[ind_mask_dh])
...
def propagate(pupil_phase_dm,pin=True,ph=False,norm=True,m_object=5,t_int=10e-3,Dtel=10,llowfs=False):
	...
	if norm==True: #normalize by summation; use for control
	...
	if norm==False: #normalize by peak lyot value; use for plotting contrast
...
scc=lambda im: image_to_pupil(Im_mask*np.fft.fftshift(pupil_to_image(im+0j))) #isolate MTF side lobe, but don't shift to center, as calibrated in IM; apply grey scale (butterworth) DH mask to control region
...
def make_IM(i):
	if freq_loop[i]<(e+1) or freq_loop[i]>N_act/2.*beam_ratio: #masking sine waves that fall within 1 lambda/D of the PSF core to prevent contamination/improve linearity of the SCC, and those that are in the outer corners of the square dark hole to minize noise amplification in closed-loop (fringe visibility is too low in this region)
		...
		np.save(refdir+'/'+str(i),np.ndarray.flatten(np.array([np.real(imdum[ind_mask_dh]),np.imag(imdum[ind_mask_dh])])))
		np.save(refdir+'/'+str(i+len(freq_loop)),np.ndarray.flatten(np.array([np.real(imdum[ind_mask_dh]),np.imag(imdum[ind_mask_dh])])))
	else:
		Im_cos,im_cos,cos=funcos(freq_loop[i],pa_loop[i])
		Im_sin,im_sin,sin=funsin(freq_loop[i],pa_loop[i])
		np.save(refdir+'/'+str(i),np.ndarray.flatten(np.array([np.real(Im_cos[ind_mask_dh]),np.imag(Im_cos[ind_mask_dh])])))
		np.save(refdir+'/'+str(i+len(freq_loop)),np.ndarray.flatten(np.array([np.real(Im_sin[ind_mask_dh]),np.imag(Im_sin[ind_mask_dh])])))
...
def make_covinvrefj(i):
	covinv=np.load('covinv_'+refdir+'.npy')
