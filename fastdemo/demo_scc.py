...
if glob('ph_diff.npy')==['ph_diff.npy']:
	...
	np.save('ph_diff.npy',phout_diff)
...
if glob('gopt.npy')==['gopt.npy']:
	...
	for i in list(range(niter)):
		...
		print('open-loop iteration:'+str(i)+' of '+str(niter))
	...
	np.save('ol_coeffs.npy',coeffsolarr)
	np.save('ol_coeffs_lowfs.npy',coeffslowfsolarr)
	np.save('gopt.npy',gopt)
	np.save('gopt_lowfs.npy',goptlowfsarr)
	np.save('goptim.npy',optgim)
...
np.save('phdm.npy',phdm)
np.save('phatmin.npy',phatmin)
np.save('imin.npy',imin)
