...
def make_kolmogorov_noise(wavefronterror, imagepix, wavelength, powerlaw=-5./3.):
	'''
	make kolmogorov noise with -5/3 power law (or other, as specified in the `powerlaw` parameter):
	...
	amplitude = (xy+1)**(powerlaw) #amplitude central value in xy grid is one, so max val is one in power law, everything else lower
