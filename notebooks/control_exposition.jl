...
# ╔═╡ 3d678aa6-ebf1-11eb-1a6e-bf3dd408d7fa
begin
...
# ╔═╡ 827fceb2-6fb8-4fa3-8731-6b2d918dc6ce
md"# LQG Control Demo
...
# ╔═╡ f194f130-231d-4e53-87e4-e050ed8f0ecc
md"Of the above imports, the only one that might not be standard is the `dare` function from `ControlSystems.jl`. This solves a general **d**iscrete **a**lgebraic **R**iccati **e**quation, which will give us our optimal gain values. This can be done in Python too, with `scipy.linalg.solve_discrete_are` and essentially the same syntax."

# ╔═╡ d1d9934b-d6fa-4845-8d6e-a7a6f9503fc4
md"
My main reference is [these](https://stanford.edu/class/ee363/lectures/lqg.pdf) lecture slides from Stanford's EE 363.
...
# ╔═╡ 2a1153bc-0fa0-46b9-a0ae-5dd8290c1ed2
begin
	# set up the problem
	s = 5; # state size
...
# ╔═╡ f896e33c-8825-4a42-a581-fa39bb12efef
md"
...
# ╔═╡ fee81023-c079-4915-b5d8-3718ef663bba
begin
...
begin
	...
	plot(1:nsteps, states, labels=["x1" "x2" "x3" "x4" "x5"], xlabel="Time step", ylabel="State variable", title="Zero input, rms err = $(rms(states))")
end

# ╔═╡ 87aee6fa-04fb-41ef-9b4f-6875df0fb972
md"Adding noise into this, we see why control is necessary; while paths seem relatively stable, they're veering far off and not staying close to 0 at all."

# ╔═╡ c25e0eb4-768b-4363-bd38-ea6fc997ba13
# uncontrolled path, zero inputs
begin
	local states = zeros(Float64, nsteps, s)
	...
	plot(1:nsteps, states, labels=["x1" "x2" "x3" "x4" "x5"], xlabel="Time step", ylabel="State variable", title="Zero input with noise, rms err = $(rms(states))")
end

# ╔═╡ 41b5cb88-5514-4aa9-9a2d-5ef3c1662cf1
md"
...
# ╔═╡ cee98da4-6f56-4926-8641-8be16c6966c0
...
begin
	...
	plot(1:nsteps, states, labels=["x1" "x2" "x3" "x4" "x5"], xlabel="Time step", ylabel="State variable", title="Random inputs with noise, rms err = $(rms(states))")
end

# ╔═╡ e78ef97f-133f-48e9-abf0-5e909e3c585d
md"Now, we bring in the controller! We first run it without any noise. We see here that all the state variables go to 0 very quickly, as you would expect from no process or measurement noise and full-state observations."

# ╔═╡ 019e4215-e8f3-46f9-880f-25bc9c2c20cc
...
begin
	...
	plot(1:nsteps, states, labels=["x1" "x2" "x3" "x4" "x5"], xlabel="Time step", ylabel="State variable", title="Noiseless LQG control, rms err = $(rms(states))")
end

# ╔═╡ 5920d169-c4c5-4ec7-a304-ea6a7273a874
md"Now we add noise back in. Compare this to the case of no control - we're staying much closer to the origin now! It's still not as good as it could be, because we've also told the controller it has to compensate for measurement noise but haven't put any in, causing it to overcorrect. We'll see this solve itself in a bit, but you could rerun the AREs above with $V = 0$ to see how that affects it."

# ╔═╡ 76b692ac-4b30-434f-aa74-839c1f50b8c4
...
begin
	...
	plot(1:nsteps, states, labels=["x1" "x2" "x3" "x4" "x5"], xlabel="Time step", ylabel="State variable", title="LQG with measurement noise, rms err = $(rms(states))")
end

# ╔═╡ 04b342c3-412a-430a-a6e8-159e3e789fe2
md"Correcting *this* measurement noise is actually a different algebraic Riccati equation than the one we solved above - let's quickly compute a Kalman gain that'll help us solve this problem!"

# ╔═╡ 44d803d7-5213-4652-8ff3-008cc010238f
begin
	Pk_full = dare(A, I, W, V)
	L_full = Pk_full * inv(Pk_full + R) # Kalman gain with C = I
end

# ╔═╡ adf69d9f-d49e-41af-94e4-2b94ef64e2cc
md"From here on out, we store two arrays of the states: the true values in `states`, so we can keep track of how well we're controlling them, and our best-estimate values in `states_hat`, to use as control input."

# ╔═╡ e428efc6-8e6a-4f26-ae1b-2836608a9afc
# full state observation plus noise and Kalman correction, LQG control
begin
	local states = zeros(Float64, nsteps, s)
	...
	for i in 2:nsteps
		...
		# you could just say innovation = rand(measure_dist) here, but y is separate for clarity
		states_hat[i,:] = y + L_full * innovation
		states[i,:] = states_hat[i,:] + rand(process_dist)
	end
	plot(1:nsteps, states, labels=["x1" "x2" "x3" "x4" "x5"], xlabel="Time step", ylabel="State variable", title="LQG with full Kalman, rms err = $(rms(states))")
end

# ╔═╡ d5d52768-b8f9-44f9-99ce-a00b07fbe94e
md"Having built all of this up, let's go to the full machinery with partial-state observations!"

# ╔═╡ c9130e18-ec1c-4091-b1ec-f5ff53e61063
# full state observation plus noise and Kalman correction, LQG control
begin
	local states = zeros(Float64, nsteps, s)
	local states_hat = zeros(Float64, nsteps, s)
	...
	plot(1:nsteps, states, labels=["x1" "x2" "x3" "x4" "x5"], xlabel="Time step", ylabel="State variable", title="LQG with partial Kalman, rms err = $(rms(states))")
end
