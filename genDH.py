...
undersize=27/32 #assuming BLANK of the 32 actuators are illuminated
...
imxcenini,imycenini,freqfudge=157,185,0.98
...
beam_ratio=6.6 #setting this as a hard value at the momnet as determined by the vf function below
...
maxld,minld=12,3 #maximum and minimum radius from which to dig a dark hole (sqrt(2) times larger for the maximum and the DH corners)
#ylimld=11 #how much to go +/- in y for the DH
#ind_dh=np.where(np.logical_and(np.logical_and(np.logical_and(Im_grid[1]-imxcen<(0+1)*beam_ratio,Im_grid[1]-imxcen>-(maxld+1)*beam_ratio),np.logical_and(Im_grid[0]-imycen<(maxld+1)*beam_ratio,Im_grid[0]-imycen>-(maxld+1)*beam_ratio)),Im_rho>(minld-1)*beam_ratio)) #full possible area for half DH
...
ind_dh=np.where(np.logical_and(np.logical_and(Im_grid[1]-imxcen<-(minld)*beam_ratio,Im_grid[1]-imxcen>-(maxld)*beam_ratio),np.logical_and(Im_grid[0]-imycen<(maxld)*beam_ratio,Im_grid[0]-imycen>-(maxld)*beam_ratio)))
dh_mask[ind_dh]=1.
...
def tune_beam_ratio(beam_ratio,i,v=True): #function to determine the emperical beam ratio based on where the centering the binary mask on it's respective sine spots; the result should modify the beam_ratio variable back above the dh_mask code
	...
	#indloop=np.where(np.logical_and(np.logical_and(np.logical_and(loopy-imycen<maxld*beam_ratio,loopy-imycen>-maxld*beam_ratio),np.logical_and(loopx-imxcen<0*beam_ratio,loopx-imxcen>-maxld*beam_ratio)),freq_loop>minld))
...
for nit in range(numiter):
	imin=stack(1000) #larger number of stacks increases the amount by which you can gain...
	tar=scc_imin(imin)
	coeffs=np.dot(cmd_mtx,tar)
